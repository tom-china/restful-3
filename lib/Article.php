<?php
// 引入错误码类
require_once __DIR__."/ErrorCode.php";
/**
 * 文章类
 * Created by PhpStorm.
 * User: Administrator
 * Date: 2017/5/4 0004
 * Time: 下午 3:20
 */
class Article {
    /**
     * 数据库句柄
     */
    private $_db;

    /**
     * 构造函数,数据库连接句柄
     * Article constructor.
     * @param PDO $_db The database
     */
    public function __construct($_db)
    {
        $this->_db = $_db;
    }

    /**
     * 创建文章
     *
     * @param string $title 文章标题 The title
     * @param string $content 文章内容 The content
     * @param int $userId 用户ID The user identifier
     * @return array
     */
    public function create($title, $content, $userId)
    {
        if (empty($title)) {
            Response::err(ErrorCode::ARTICLE_TITLE_CANNOT_EMPTY, '文章标题不能为空');
        }
        if (empty($content)) {
            Response::err(ErrorCode::ARTICLE_CONTENT_CANNOT_EMPTY, '文章内容不能为空');
        }
        $addtime = time();
        // 数据写入
        $sql = 'INSERT INTO `article`(`title`,`content`,`addtime`,`user_id`)
           VALUES(:title,:content,:addtime,:user_id)';
        $stmt = $this->_db->prepare($sql);
        $stmt->bindParam(':title', $title);
        $stmt->bindParam(':content', $content);
        $stmt->bindParam(':addtime', $addtime);
        if (!$stmt->execute()) {
            Response::err(ErrorCode::ARTICLE_CREATE_FAIL, '发布文章失败');
        }
        return array(
            'articleId' => $this->_db->lastInsertId(),
            'title' => $title,
            'content'   => $content
        );
    }

    /**
     * 查看文章
     *
     * @param string $articleId The article identifier
     * @return mixed
     */
    public function view($articleId) {
        if (empty($articleId)) {
            Response::err(ErrorCode::ARTICLE_ID_CANNOT_EMPTY, '文章ID不能为空');
        }
        // 查询数据
        $sql = 'SELECT * FROM `article` WHERE `id`=:id LIMIT 1';
        $stmt = $this->_db->prepare($sql);
        $stmt->bindParam(':id',$articleId);
        $stmt->execute();
        $article = $stmt->fetch();
        if (empty($article)) {
            Response::err(ErrorCode::ARTICLE_NOT_FOUND, '文章不存在');
        }
        return $article;
    }

    /**
     * 编辑文章
     *
     * @param Integer $articleId The article identifier
     * @param String $title The title
     * @param String $content The content
     * @param String $userId The user identifier
     * @return array|mixed
     */
    public function edit($articleId,$title,$content,$userId) {
        $article = $this->view($articleId);
        if ($article['user_id'] !== $userId) {
            Response::err(ErrorCode::PERMISSSION_DENIED, '你无权编辑文章');
        }
        $title = empty($title) ? $article['title'] : $title;
        $content = empty($content) ? $article['content'] : $content;
        if ($title === $article['title'] && $content === $article['content']) {
            return $article;
        }
        // 更新数据
        $sql = 'UPDATE `article` SET `title` =:title,`content`=:content WHERE `id`=:id';
        $stmt = $this->_db->prepare($sql);
        $stmt->bindParam(':title', $title);
        $stmt->bindParam(':content',$content);
        $stmt->bindParam(':id', $articleId);
        if (!$stmt->execute()) {
            Response::err(ErrorCode::ARTICLE_EDIT_FAIL, '编辑失败');
        }
        return array(
            'id'        => $articleId,
            'title'     => $title,
            'content'   => $content,
            'addtime'   => $article['addtime'],
            'userId'    => $userId
        );
    }

    /**
     * 删除文章
     *
     * @param Integer $userId The user identifier
     * @param Integer $articleId The article identifier
     * @return bool
     * @throws Exception
     */
    public function delete($userId, $articleId) {
        $article = $this->view($articleId);
        if ($article['user_id'] !== $userId) {
            Response::err(ErrorCode::PERMISSSION_DENIED, '您无权操作');
        }
        // 删除文章
        $sql = 'DELETE FROM `article` WHERE `id` = :id AND `user_id` = :user_id';
        $stmt = $this->_db->prepare($sql);
        $stmt->bindParam(':id', $articleId);
        $stmt->bindParam(':user_id', $userId);
        if (!$stmt->execute()) {
            throw new Exception("文章删除失败", ErrorCode::ARTICLE_DELETE_FAIL);
        }
        return $stmt->execute();
    }

    /**
     * 读取文章列表
     *
     * @param Integer $userId The user identifier
     * @param int $page The page
     * @param int $size The size
     */
    public function getList($userId, $page=1, $size=10) {
        if ($size > 100) {
            throw new Exception("分页大小最大为100", ErrorCode::PAGE_SIZE_TO_BIG);
        }
        // 查询数据
        $sql = 'SELECT * FROM `article` WHERE user_id = :user_id LIMIT :limit, :offset';
        $limit = ($page - 1) * $size;
        $limit = $limit < 0 ? 0 : $limit;
        $stmt = $this->_db->prepare($sql);
        $stmt->bindParam(':user_id', $userId);
        $stmt->bindParam(':limit', $limit);
        $stmt->bindParam(':offset', $size);
        if (!$stmt->execute()) {
            throw new Exception("Error Processing Request", 1);
        }
        $data = $stmt->fetchAll();
        return $data;
    }
}